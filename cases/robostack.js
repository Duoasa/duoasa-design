...
function initRobostackTypewriter() {
  const target = document.querySelector(".robostack-typewriter[data-type-text]");
  if (!target) return;

  const fullText = target.dataset.typeText || target.textContent.trim();
  if (!fullText) return;
  ...
  let timeoutId = 0;
  let runId = 0;
  const keyDelays = [132, 78, 92, 146, 68, 104, 88, 126, 82];
  ...
  const hitKey = (index, currentRun) => {
    if (currentRun !== runId) return;

    target.textContent = fullText.slice(0, index + 1);
    target.classList.add("is-key-hit");
    window.setTimeout(() => target.classList.remove("is-key-hit"), 54);

    if (index + 1 >= fullText.length) {
      schedule(() => {
        if (currentRun !== runId) return;
        target.textContent = "";
        schedule(() => startTyping(), 520);
      }, 3600);
      return;
    }

    const punctuationPause = index === 3 ? 96 : 0;
    const humanVariance = Math.round(Math.random() * 58);
    schedule(() => hitKey(index + 1, currentRun), keyDelays[index % keyDelays.length] + humanVariance + punctuationPause);
  };

  const startTyping = () => {
    runId += 1;
    target.textContent = "";
    schedule(() => hitKey(0, runId), 540);
  };

  const startWhenHeroIsReady = () => {
    if (document.body.classList.contains("quest-ready")) {
      startTyping();
      return;
    }

    const observer = new MutationObserver(() => {
      if (!document.body.classList.contains("quest-ready")) return;
      observer.disconnect();
      startTyping();
    });
    ...
    schedule(() => {
      observer.disconnect();
      if (!target.textContent || target.textContent === fullText) startTyping();
    }, 2400);
  };

  target.textContent = "";
  startWhenHeroIsReady();
